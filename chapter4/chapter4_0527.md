...
* タイプ，長さ（2バイト）  
	...
	* IEEE802.3 : 長さフィールド  
		フレーム長を保持している．
		受信者にとってフレームをどう処理すべきか判断する方法は， **LLC（Logical Link Control : 論理リンク制御）** 
...
このシステムの中心は，図4-17(b)(p280)のように，すべてのポートをつなぐ高速なバックプレーンを備えた **スイッチ（switch）** である．
...
ハブではすべての局が同一の **衝突ドメイン（collision domain）** 
...
* ハブに対するスイッチのメリット
	...
	* トラフィックは目的外のポートへ送信されないため，セキュリティが向上する．
		* LANインタフェースには **プロミスキャス・モード（promiscuous mode）** があり，自分宛てでないフレームも受け取ることができる．
